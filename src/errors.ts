...
export class UnknownHTTPResponseError extends HTTPResponseError<ClientErrorCode.ResponseError> {
  ...
  /**
   * Value of the `cf-ray` response header, when present. A response carrying a
   * Ray ID but no Notion error body was produced by Notion's edge proxy rather
   * than by the API itself — typically a security rule blocking the request
   * before it reached Notion. Quote this ID when contacting Notion support.
   */
...
/**
 * Builds the message for a response the Notion API did not generate.
 *
 * A `cf-ray` header means the response came back through Notion's edge proxy.
 * Combined with a body that is not a Notion error, that indicates the request
 * was answered at the edge — most often a security rule blocking it before it
 * reached the API — so the message says so explicitly and surfaces the Ray ID
 * rather than leaving callers to infer it from an HTML body.
 */
function buildUnknownResponseMessage(args: {
  ...
  const { status, contentType, ray_id } = args
  ...
  return (
    `${base}. The response was not generated by the Notion API${contentTypeNote}; ` +
    `it was returned by Notion's edge proxy, which usually means the request was ` +
    `blocked by a network security rule before reaching the API. ` +
    `Cloudflare Ray ID: ${ray_id} — quote this when contacting Notion support.`
  )
}
