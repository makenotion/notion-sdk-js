...
describe("Notion SDK Client", () => {
  ...
  describe("edge-proxy responses", () => {
    ...
    it("surfaces the Ray ID and edge origin for an HTML block page", async () => {
      mockFetch.mockResolvedValue(
        mockRawResponse({
          status: 403,
      ...
      try {
        await notion.comments.list({ block_id: TEST_BLOCK_ID })
        assert.fail("Expected error to be thrown")
      } catch (error) {
        ...
        expect(error.message).toContain(
          "was not generated by the Notion API (content-type: text/html; charset=UTF-8)"
        )
        expect(error.message).toContain(
          "Cloudflare Ray ID: 9a1b2c3d4e5f6789-SJC"
        )
    ...
    it("keeps the generic message when no Ray ID is present", async () => {
      mockFetch.mockResolvedValue(
        mockRawResponse({
          status: 502,
    ...
    it("prefers a Notion error body over the edge message when both are present", async () => {
      mockFetch.mockResolvedValue(
        mockRawResponse({
          status: 403,
          body: JSON.stringify({
